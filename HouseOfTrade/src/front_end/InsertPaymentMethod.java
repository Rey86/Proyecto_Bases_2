...
public class InsertPaymentMethod extends javax.swing.JDialog {
    private Integer id_paymentmethod;

    public InsertPaymentMethod(java.awt.Frame parent, boolean modal, Integer id_paymentmethod) {
        super(parent, modal);
    ...
    public void initial(){
        try{
            if(id_paymentmethod > 0){
                ResultSet r = logic_connection.DataBaseConnection.getPaymentMethod(id_paymentmethod);
                if(r.next()) {
                    jLabelID.setText(String.valueOf(r.getInt("ID_PAYMENTMETHOD")));
    ...
    private void jButtonAcceptActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAcceptActionPerformed
        if (!jTextFieldName.getText().equals("")){
            try{
                if(id_paymentmethod > 0){
                    logic_connection.DataBaseConnection.setPaymentMethod(Integer.valueOf(jLabelID.getText()), jTextFieldName.getText());
                    dispose();
                }
                else{
                    logic_connection.DataBaseConnection.insertPaymentMethod(jTextFieldName.getText());
                    dispose();
                }
            }
            catch (SQLException e){
                ...
                JOptionPane.showMessageDialog(this, "The ID Payment Method box is empty", "Watch out", JOptionPane.ERROR_MESSAGE);
            }
        } else {
