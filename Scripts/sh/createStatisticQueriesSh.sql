...
BEGIN
	...
	SELECT c.name category,round(100*count(product) , 2) percentage, count(product) total
    from product p 
...
BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLEXCEPTION SELECT 'SQLException encountered' Message; 
    DECLARE EXIT HANDLER FOR SQLSTATE '23000' SELECT 'SQLSTATE 23000' ErrorCode;
	SELECT g.name gender, round(100*count(product) , 2) percentage, count(product) total
    FROM product p
...
BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLSTATE '23000' SELECT 'SQLSTATE 23000' ErrorCode;
    -- check this
	SELECT g.name gender, round(100*sum(pr.price) , 2) percentage, sum(pr.price) total 
    FROM purchase pu
